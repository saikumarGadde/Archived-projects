...
def main():
	...
	## Hyper parameter search ranges are: 
	orders = [ 1,2,3 ]
			...
			for gamma in gammas:
				for epsilon in epsilons:
					...
					for i in range(500):
						...
						output.append(q_learning(alpha, gamma, epsilon, order, 2))

					output_array = np.array(output)
					averages = np.mean(output_array, axis = 0)
					averages_100 = np.mean(averages)
	...
	for i in range(500):
		output = q_learning(optimal_alpha, optimal_gamma, optimal_epsilon, optimal_order, 2)
		outputs.append(output)
	outputs = np.array(outputs)

	plt.figure(1)
	plt.plot(np.mean(outputs,axis=0))
	plt.title("Q_Learning for the optimal hyper parameters")
	plt.gca().set_ylim([-1000,0])
	plt.show()

	plt.figure(2)
	plt.errorbar(range(60), np.mean(outputs,axis=0), yerr=np.std(outputs,axis=0))
	plt.title("Q_learning error bars for the optimal hyper parameter optimization")
	plt.gca().set_ylim([-1000,0])
	plt.show()
...
def getFeatureVector(state , velocity ,no_features, action, order, dimension):
	...
	c = fourier_basis(order, dimension)
...
def q_learning(learning_rate, discount_factor, epsilon, degree, dimension):
